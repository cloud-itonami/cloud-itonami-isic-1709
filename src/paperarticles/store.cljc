(ns paperarticles.store
  "SSoT for the ISIC 1709 (other paper and paperboard articles)
  plant-operations coordination actor, behind a `Store` protocol so
  ...
  for dev/tests/demo, no deps. Mirroring `cloud-itonami-isic-1701`/
  `cloud-itonami-isic-1702`'s own paper verticals (docs/adr/0001-
  architecture.md Decision 1), this vertical is self-contained (no
  external paper-articles capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             paper-article product-grade/quantity/
                             quality (basis-weight/moisture-content)
                             record -- see README for the chosen
                             illustration (molded-pulp/paperboard
                             tableware). `:verified?` marks whether the
                             ...
                             `:shipped-quantity-units` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a pulp-molding-press/die-cutter/
                             converting-line unit's own record.
    ...
    - `maintenance`       -- a scheduled converting-line
                             maintenance-window DRAFT against a piece
                             of equipment (`paperarticles.registry`'s
    ...
    - `shipments`         -- a proposed outbound paper-article
                             shipment DRAFT (`paperarticles.registry`'s
                             `register-shipment`).
  ...
  audit trail a plant owner or tableware/filter-paper/wallpaper buyer
  trusting this coordinator needs."
  (:require [paperarticles.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (paperarticles.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (paperarticles.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :molded-pulp-plate :customer "buyer-tableware-north"
                :quantity-units 5000.0 :basis-weight-gsm 220.0 :moisture-content-pct 6.0
                :verified? true :registered? true
                :shipped-quantity-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :coated-paperboard-cup :customer "buyer-tableware-east"
                :quantity-units 800.0 :basis-weight-gsm 260.0 :moisture-content-pct 5.5
                :verified? true :registered? true
                :shipped-quantity-units 750.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :molded-pulp-bowl :customer "buyer-tableware-south"
                :quantity-units 6000.0 :basis-weight-gsm 200.0 :moisture-content-pct 6.5
                :verified? false :registered? false
                :shipped-quantity-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :pulp-molding-press
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :die-cutter
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `paperarticles.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `paperarticles.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-quantity-units]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:quantity-units value) 0.0))))))))
        result)
      ...
      ;; above (also what `pulppaper`-style siblings expose as their
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged quantity
  -- HARD hold), one UNVERIFIED/unregistered batch (blocks any
  shipment coordinated against it); one verified+registered
  pulp-molding-press unit (schedulable for maintenance), one
  UNVERIFIED/unregistered die-cutter unit (blocks any maintenance
