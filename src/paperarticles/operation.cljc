(ns paperarticles.operation
  "PaperArticlesOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (PaperArticlesAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Paper Articles Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [paperarticles.advisor :as advisor]
...
(defn build
  "Compiles a PaperArticlesOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `paperarticles.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PaperArticlesAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Paper Articles Plant Operations Governor -- independent censor.
      (g/add-node :govern
