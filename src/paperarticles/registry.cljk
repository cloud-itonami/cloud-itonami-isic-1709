...
(defn shipment-quantity-exceeded?
  ...
  (let [capacity (:quantity-units batch)
    ...
    (and (number? capacity)
         ...
         (> (Math/round (* 10000 (+ (double so-far) (double new-quantity-units))))
            (Math/round (* 10000 (double capacity))))))) 

(defn shipment-quantity-exceeded-checkable?
  "Can `batch`'s headroom actually be computed for `new-quantity-units`?
  ...
  (boolean (and (map? batch)
                ...
                (number? (:shipped-quantity-units batch 0.0))
                (number? new-quantity-units))))
