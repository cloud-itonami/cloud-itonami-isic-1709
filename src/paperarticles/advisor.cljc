(ns paperarticles.advisor
  "PaperArticlesAdvisor -- the *contained intelligence node* for the
  ISIC 1709 (other paper and paperboard articles) plant-operations
  coordination actor. Chosen concrete illustration: molded-pulp/
  paperboard paper-tableware manufacturing (paper plates/bowls/trays,
  coated and uncoated paper cups) -- see README for how this
  illustration stands in for this category's other n.e.c. product
  lines (filter paper, wallpaper base stock).

  It normalizes production-batch patches (product grade/quantity/
  basis-weight/moisture-content), drafts a pulp-molding-press/die-
  cutter/converting-line maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound paper-article shipment coordination proposal against a
  ...
  a committed record and NEVER a real converting-line-equipment
  actuation or freight dispatch. Every output is censored downstream
  by `paperarticles.governor` before anything touches the SSoT -- see
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `paperarticles.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct converting-
  ...
  contract every caller of this actor agrees to) -- `paperarticles.
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [paperarticles.registry :as registry]
            [paperarticles.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's grade, quantity,
  basis-weight/moisture-content readings or verification status. High
...
(defn- schedule-maintenance
  "Draft a pulp-molding-press/die-cutter/converting-line maintenance-
  window scheduling proposal against a piece of equipment. The
  advisor reports what it can see (equipment verified?/registered?)
  in its rationale, but `paperarticles.governor` NEVER trusts this
  report -- it independently re-derives verified?/registered? from
  the equipment's own stored fields before any commit is possible."
...
(defn- flag-safety-concern
  "Draft an equipment-hazard/quality-defect safety-concern flag. ALWAYS
  ...
  `paperarticles.phase`: no phase ever adds this op to a phase's
  `:auto` set; `paperarticles.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound paper-article shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed quantity -- it does NOT invent one, and `paperarticles.
  governor` NEVER trusts it: it independently recomputes whether the
  batch's own cumulative-shipped quantity plus this claim would exceed
  the batch's own recorded quantity before any commit is possible."
  [db {:keys [subject value]}]
  (let [batch-id (:batch-id value)
        ...
        over-quantity? (and b (registry/shipment-quantity-exceeded?
                               b (:quantity-units value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:quantity-units value) " 単位)"
...
(def ^:private system-prompt
  (str "あなたは製紙加工品プラント運用コーディネーターの助言者です。"
       ...
       "製紙加工コンバーティングライン設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `paperarticles.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :paperarticles-advisor-proposal
   :op         (:op request)
