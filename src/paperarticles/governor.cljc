(ns paperarticles.governor
  "Paper Articles Plant Operations Governor -- the independent
  compliance layer that earns the PaperArticlesAdvisor the right to
  ...
  maintenance proposal secretly tries to directly CONTROL pulp-
  molding-press/die-cutter/converting-line equipment (rather than
  ...
  coordination proposal and becomes direct converting-line-equipment
  control, so this MUST be a separate system able to *reject* a
  proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:paper-articles-plant-operations-
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    7. Shipment quantity exceeded  -- for `:coordinate-shipment`,
                                       INDEPENDENTLY recompute whether
                                       the batch's own recorded
                                       `:shipped-quantity-units` plus
                                       the proposal's own claimed
                                       `:quantity-units` would exceed
                                       the batch's own recorded
                                       `:quantity-units`
                                       (`paperarticles.registry/
    ...
    8. Invalid grade               -- for `:log-production-batch`, if
                                       the patch declares a `:grade`
                                       outside the closed known set
                                       (`paperarticles.registry/grade-
                                       valid?`), the batch record is
                                       rejected rather than let a
                                       fabricated grade through.
    9. Invalid basis weight        -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:basis-weight-gsm` that is not
                                       a physically plausible grammage
                                       reading
                                       (`paperarticles.registry/basis-
                                       weight-valid?`), the batch
                                       record is rejected rather than
                                       let fabricated/sensor-error
                                       data through.
   10. Invalid moisture content    -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:moisture-content-pct` that is
                                       not a physically plausible
                                       reading
                                       (`paperarticles.registry/
                                       moisture-content-valid?`), the
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct converting-
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct pulp-molding-press/die-cutter/converting-line
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は製紙加工コンバーティングライン設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- invalid-basis-weight-violations
  "For `:log-production-batch`, if the patch declares a
  `:basis-weight-gsm` that is not a physically plausible grammage
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [gsm (:basis-weight-gsm (:value proposal))]
      (when (and (some? gsm) (not (registry/basis-weight-valid? gsm)))
        [{:rule :invalid-basis-weight
          :detail (str gsm "gsm は物理的に妥当な坪量の範囲外")}]))))

(defn- invalid-moisture-content-violations
  "For `:log-production-batch`, if the patch declares a
  `:moisture-content-pct` that is not a physically plausible reading,
  reject rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [pct (:moisture-content-pct (:value proposal))]
      (when (and (some? pct) (not (registry/moisture-content-valid? pct)))
        [{:rule :invalid-moisture-content
          :detail (str pct "% は物理的に妥当な含水率の範囲外")}]))))

(defn check
  "Censors a PaperArticlesAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-basis-weight-violations request proposal)
                           (invalid-moisture-content-violations request proposal)))
