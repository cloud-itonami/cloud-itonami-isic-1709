(ns paperarticles.registry-test
  (:require [clojure.test :refer [deftest is]]
            [paperarticles.registry :as r]))
...
(deftest small-shipment-within-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 5000.0 :shipped-quantity-units 1000.0} 500.0))))

(deftest shipment-that-pushes-past-quantity-exceeds
  (is (true? (r/shipment-quantity-exceeded?
              {:quantity-units 800.0 :shipped-quantity-units 750.0} 100.0))))

(deftest shipment-exactly-at-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 800.0 :shipped-quantity-units 750.0} 50.0))
...
(deftest known-grades-are-valid
  (doseq [g [:molded-pulp-plate :molded-pulp-bowl :molded-pulp-tray
...
(deftest typical-basis-weight-is-valid
  (is (r/basis-weight-valid? 220.0))
  (is (r/basis-weight-valid? 0.0))
  (is (r/basis-weight-valid? 260.0))
  (is (r/basis-weight-valid? 500.0)))

(deftest negative-basis-weight-is-invalid
  (is (not (r/basis-weight-valid? -1.0))))

(deftest excessive-basis-weight-is-invalid
  (is (not (r/basis-weight-valid? 5000.0)))
  (is (not (r/basis-weight-valid? 500.01))))

(deftest non-numeric-or-missing-basis-weight-is-invalid
  (is (not (r/basis-weight-valid? nil)))
  (is (not (r/basis-weight-valid? "220.0"))))

;; ----------------------------- moisture-content-valid? -----------------------------

(deftest typical-moisture-content-is-valid
  (is (r/moisture-content-valid? 6.0))
  (is (r/moisture-content-valid? 0.0))
  (is (r/moisture-content-valid? 5.5))
...
(deftest excessive-moisture-content-is-invalid
  (is (not (r/moisture-content-valid? 90.0)))
  (is (not (r/moisture-content-valid? 15.01))))

(deftest non-numeric-or-missing-moisture-content-is-invalid
  (is (not (r/moisture-content-valid? nil)))
  (is (not (r/moisture-content-valid? "6.0"))))
