(ns paperarticles.operation-test
  "Smoke tests for the compiled PaperArticlesOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `paperarticles.governor-contract-test`; the Store contract in
  `paperarticles.store-contract-test`."
...
(deftest test-actor-builds
  (testing "PaperArticlesOperationActor can be built with a store"
    (let [s (store/mem-store)
          actor (op/build s)]
      (is (not (nil? actor))))))

(deftest test-production-batch-logging-proposal
  (testing "Proposing a production-batch log auto-commits when clean (phase 3, no physical risk)"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :severity :moderate :description "成形プレス駆動部で異音"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
