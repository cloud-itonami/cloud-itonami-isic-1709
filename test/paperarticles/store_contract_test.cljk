(ns paperarticles.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `paperarticles.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:grade :molded-pulp-plate}})
    (is (= :molded-pulp-plate (:grade (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-quantity
  (let [s (seeded)]
    ...
    (is (= 1500.0 (:shipped-quantity-units (store/batch s "batch-001")))
